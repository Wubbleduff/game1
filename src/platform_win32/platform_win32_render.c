...
void platform_win32_render_game_state(struct GameState* game_state)
{
...

    {
        ...
        ASSERT(fb_width * fb_height >= 8, "Frame buffer too small.");
        for(u64 i = 0; i < fb_width * fb_height - 8; i += 8)
        {
            _mm256_store_si256(
                    (__m256i*)(fb + i),
                    color8);
        }
        for(u64 i = fb_width * fb_height - 8; i < fb_width * fb_height; i++)
        {
            fb[i] = color;
        }
    }
...

    {
        ...
        // TODO(mfritz) Hook this up for all players.
        const f32 player_x = game_state->player_pos_x[0];
        const f32 player_y = game_state->player_pos_y[0];
        ...
        {
            for(u64 i_x = 0; i_x < fb_width - 8; i_x += 8)
            {
                ...
                {
                    {
                        __m256 q0_x = wall_x[i_wall][0];
                        __m256 q0_y = wall_y[i_wall][0];
                        __m256 q1_x = wall_x[i_wall][1];
                        __m256 q1_y = wall_y[i_wall][1];
                        __m256 q_nx = wall_nx[i_wall][0];
                        __m256 q_ny = wall_ny[i_wall][0];

                        __m256 d0 = _mm256_fmadd_ps(q0_x, p_nx, _mm256_mul_ps(q0_y, p_ny));
                        __m256 d1 = _mm256_fmadd_ps(q1_x, p_nx, _mm256_mul_ps(q1_y, p_ny));
                        __m256 d2 = _mm256_fmadd_ps(_mm256_mul_ps(q0_x, _mm256_set1_ps(-1.0f)), q_nx, _mm256_mul_ps(_mm256_mul_ps(q0_y, _mm256_set1_ps(-1.0f)), q_ny));
                        __m256 d3 = _mm256_fmadd_ps(_mm256_sub_ps(p1_x, q0_x), q_nx, _mm256_mul_ps(_mm256_sub_ps(p1_y, q0_y), q_ny));

                        __m256 d4 = _mm256_fmadd_ps(p1_x, q_nx, _mm256_mul_ps(p1_y, q_ny));

                        __m256 intersect = _mm256_and_ps(
                                _mm256_xor_ps(d0, d1),
                                _mm256_xor_ps(d2, d3));
                        __m256 mask = _mm256_and_ps(intersect, d4);

                        occluded = _mm256_or_ps(occluded, mask);
                    }

                    {
                        __m256 q0_x = wall_x[i_wall][1];
                        __m256 q0_y = wall_y[i_wall][1];
                        __m256 q1_x = wall_x[i_wall][2];
                        __m256 q1_y = wall_y[i_wall][2];
                        __m256 q_nx = wall_nx[i_wall][1];
                        __m256 q_ny = wall_ny[i_wall][1];
                    ...
                    {
                        __m256 q0_x = wall_x[i_wall][2];
                        __m256 q0_y = wall_y[i_wall][2];
                        __m256 q1_x = wall_x[i_wall][3];
                        __m256 q1_y = wall_y[i_wall][3];
                        __m256 q_nx = wall_nx[i_wall][2];
                        __m256 q_ny = wall_ny[i_wall][2];
                    ...
                    {
                        __m256 q0_x = wall_x[i_wall][3];
                        __m256 q0_y = wall_y[i_wall][3];
                        __m256 q1_x = wall_x[i_wall][0];
                        __m256 q1_y = wall_y[i_wall][0];
                        __m256 q_nx = wall_nx[i_wall][3];
                        __m256 q_ny = wall_ny[i_wall][3];
                ...
                __m256 out_pixels = _mm256_loadu_ps((f32*)(fb + i_y*fb_width + i_x));
                _mm256_storeu_ps(
                    (f32*)(fb + i_y*fb_width + i_x),
                    _mm256_blendv_ps(
                        out_pixels,
                        _mm256_set1_ps(0.0f),
                        occluded
    ...
    memcpy(render->dib_frame_buffer, fb, render->frame_buffer_width * render->frame_buffer_height * sizeof(fb[0]));
